...
//   { header: 'HT (F)',   dataKey: 'ht'       },
//   { header: 'TVA (F)',  dataKey: 'tva'      },
//   { header: 'TTC (F)',  dataKey: 'ttc'      },
// ]
...
//                       <Package size={24} className="mx-auto mb-2 opacity-30" />Aucune ligne
//                     </td></tr>
//                   : (vente.lignes ?? []).map(l => (
...
//       if (produit) { ajouterLigne(produit); setSearch('') }
//       else if (produitsFiltres.length === 1) { ajouterLigne(produitsFiltres[0]); setSearch('') }
//       else { setError('Produit non trouvé'); setTimeout(() => setError(''), 2000) }
//     }
//   }
...
//                       return (
//                         <button key={p.idProduit} onClick={() => ajouterLigne(p)} disabled={dispo === 0}
//                           className={`
...
//                             {p.photo
//                               ? <img src={`http://127.0.0.1:8000/storage/${p.photo}`} alt={p.reference} className="w-full h-full object-cover" />
...
//                         {l.quantite === 1 ? <Trash2 size={11} className="text-error" /> : <span className="text-xs">−</span>}
//                       </button>
//                       <span className="text-xs font-bold w-5 text-center">{l.quantite}</span>
//                       <button className="btn btn-ghost btn-xs btn-circle w-5 h-5 min-h-0" onClick={() => ajouterLigne(l.produit)}>
...
//   const { session, ouvrir, fermer } = useCaisse(user?.idUtilisateur)

//   // Montant TTC total de toutes les ventes actives
//   const montantActuel = useMemo(
//     () => ventesActives.reduce((s, v) => s + (parseFloat(v.totalTaxeComprise) || 0), 0),
//     [ventesActives]
//   )
...
//     } catch { exportTicketCaisse(v) }
//   }

//   const exportData = ventesFiltrees.map(v => ({
//     id: `#${v.idVente}`,
...
//     ht: Math.round(v.totalHorsTaxe || 0),
//     tva: Math.round(v.tva || 0),
...
//               </div>
//             )}
//             {/* Boutons caisse — visibles caissier + gérant */}
//             <div className="flex items-center gap-2">
//               {session && (
//                 <div className="text-xs text-base-content/50 text-right leading-tight">
//                   <div>Ouverture : <span className="font-semibold text-base-content/70">
//                     {new Date(session.heureOuverture).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
//                   </span></div>
//                   <div>Fond : <span className="font-semibold text-success">{fmt(session.montantOuverture)} F</span></div>
//                 </div>
//               )}
...
//                 disabled={!!session}
//                 onClick={() => ouvrir(montantActuel)}
//               >
//                 <span className="text-base">🟢</span> Ouvrir caisse
//               </button>
...
//                 onClick={fermer}
//               >
//                 <span className="text-base">🔴</span> Fermer caisse
//               </button>
//             </div>
...
//                 </button>
//               )}
//               <span className="ml-auto text-xs text-base-content/40 font-semibold">{ventesFiltrees.length} vente(s)</span>
//             </div>
//           </div>
//         </div>
...
//               <div className="join">
//                 <button className="join-item btn btn-xs btn-ghost" disabled={page === 1} onClick={() => setPage(p => p - 1)}><ChevronLeft size={13} /></button>
...
/* ════════════════════════════════════
   HOOK SESSION CAISSE
════════════════════════════════════ */
function useCaisse(userId) {
  const KEY = `caisse_session_${userId}`

  const [session, setSession] = useState(() => {
    try { return JSON.parse(localStorage.getItem(KEY)) } catch { return null }
  })

  const ouvrir = (montantActuel) => {
    const s = {
      heureOuverture: new Date().toISOString(),
      montantOuverture: montantActuel,
    }
...
function ModalDetail({ vente, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-base-100 rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">

        <div className="bg-primary text-primary-content px-5 py-4 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white/25 rounded-2xl">
              <Receipt size={18} />
            </div>
...
function ModalNouvelleVente({ produits, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
      <div className="relative bg-base-100 rounded-2xl shadow-xl w-full max-w-6xl mx-4 h-[90vh] flex flex-col overflow-hidden">
...
export default function Ventes() {
  ...
  // ── Ventes actives (déclarées avant tout useMemo qui en dépend) ──
  const ventesActives = useMemo(() => ventes.filter(v => v.statut !== 'annulee'), [ventes])
  ...
  const todayStr  = toISO(today)
  const ventesAuj = useMemo(() => ventesActives.filter(v => v.dateVente?.startsWith(todayStr)), [ventesActives, todayStr])
  const caAuj     = useMemo(() => ventesAuj.reduce((s, v) => s + (parseFloat(v.totalTaxeComprise) || 0), 0), [ventesAuj])
  const caMois    = useMemo(() => {
    const debut = new Date(today.getFullYear(), today.getMonth(), 1).toISOString().split('T')[0]
    return ventesActives.filter(v => v.dateVente >= debut).reduce((s, v) => s + (parseFloat(v.totalTaxeComprise) || 0), 0)
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          {/* ── Boutons caisse (caissier + gérant, pas gestionnaire) ── */}
          {(user?.role === 'caissier' || user?.role === 'gerant') && (
            <div className="flex items-center gap-2">
              {/* Info session active */}
              {session && (
                <div className="text-xs text-base-content/50 leading-tight text-right hidden sm:block">
                  <div>Ouvert à <span className="font-semibold text-base-content/80">
                    {new Date(session.heureOuverture).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                  </span></div>
                  <div>Fond : <span className="font-semibold text-success">{fmt(session.montantOuverture)} F</span></div>
                </div>
              ...
              <button
                className="btn btn-sm gap-1.5 btn-success"
                disabled={!!session}
                onClick={() => {
                  ouvrir(montantActuel)
                  showToast('success', 'Caisse ouverte !')
                }}
                title={session ? 'Caisse déjà ouverte' : 'Ouvrir la caisse'}
              >
                <LogIn size={14} /> Ouvrir caisse
              </button>

              <button
                className="btn btn-sm gap-1.5 btn-error"
                disabled={!session}
                onClick={() => {
                  fermer()
                  showToast('success', 'Caisse fermée.')
                }}
                title={!session ? 'Aucune caisse ouverte' : 'Fermer la caisse'}
              >
                <LogOut size={14} /> Fermer caisse
              </button>
